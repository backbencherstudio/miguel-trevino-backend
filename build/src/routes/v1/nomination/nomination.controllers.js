...
exports.deleteNomination = exports.updateNominationStatus = exports.getAllNominations = exports.getMyNominations = exports.uploadSchedule = exports.createNomination = void 0;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const baseurl_1 = require("../../../utils/baseurl");
const storage_config_1 = require("../../../config/storage.config");
const createNomination = async (request, reply) => {
    try {
        ...
        const { commodityType, assetGroup, origin, volume, destination, unit, transportMode, beginningDate, endDate, notes, userId, connection, } = request.body;
        const missingField = [
            "commodityType",
            "assetGroup",
            "origin",
...
exports.createNomination = createNomination;
const uploadSchedule = async (request, reply) => {
    try {
        ...
        if (nomination.scheduleFile) {
            const oldFilePath = path_1.default.join(storage_config_1.uploadsDir, nomination.scheduleFile);
        ...
        return reply.status(200).send({
            ...
            data: {
                ...
                scheduleFile: updatedNomination.scheduleFile
                    ? (0, baseurl_1.getImageUrl)(`/uploads/${updatedNomination.scheduleFile}`)
                    : null,
            },
        });
    }
        ...
        if (request.file?.path && fs_1.default.existsSync(request.file.path)) {
...
const getMyNominations = async (request, reply) => {
    try {
        const prisma = request.server.prisma;
        const { id } = request.user;
        const page = parseInt(request.query.page) || 1;
        const limit = parseInt(request.query.limit) || 10;
        const startDate = request.query.startDate && !isNaN(Date.parse(request.query.startDate))
            ? new Date(request.query.startDate)
            : null;
        const endDate = request.query.endDate && !isNaN(Date.parse(request.query.endDate))
        ...
        const formattedNominations = nominations.map((nomination) => ({
            ...
            scheduleFile: nomination.scheduleFile
                ? (0, baseurl_1.getImageUrl)(`/uploads/${nomination.scheduleFile}`)
                : null,
            user: nomination.user
                ? {
                    ...nomination.user,
                    avatar: nomination.user.avatar
                        ? (0, baseurl_1.getImageUrl)(`/uploads/${nomination.user.avatar}`)
                        : null,
                }
                : null,
        }));
...
exports.getMyNominations = getMyNominations;
const getAllNominations = async (request, reply) => {
    try {
        const prisma = request.server.prisma;
        const page = parseInt(request.query.page) || 1;
        const limit = parseInt(request.query.limit) || 10;
...
exports.getAllNominations = getAllNominations;
const updateNominationStatus = async (request, reply) => {
    try {
        const prisma = request.server.prisma;
        const { nominationId } = request.params;
...
exports.updateNominationStatus = updateNominationStatus;
const deleteNomination = async (request, reply) => {
    try {
        const prisma = request.server.prisma;
        const { nominationId } = request.params;
        if (!nominationId) {
...
exports.deleteNomination = deleteNomination;
//# sourceMappingURL=nomination.controllers.js.map